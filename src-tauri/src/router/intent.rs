...
- "file_search" — il veut remettre la main sur une chose qui EXISTE DÉJÀ et qui
  lui appartient : document, fichier, note, mail, présentation, tableur. Le
  critère est la possession, pas le vocabulaire : demander où est une chose, se
  plaindre de ne plus la trouver, en avoir besoin, demander si tu l'as, ou
  simplement la nommer, relèvent tous de cette intention.
...
- "mail_compose" — il veut qu'un message parte vers une PERSONNE nommée. Le
  ...
  lui est adressé, même si aucun mot de messagerie n'apparaît.
...
pub async fn classify(llm: &Arc<dyn LlmClient>, text: &str, fallback: Intent) -> Intent {
    match tokio::time::timeout(BUDGET, ask_model(llm, text)).await {
...
async fn ask_model(llm: &Arc<dyn LlmClient>, text: &str) -> Result<Option<Intent>> {
